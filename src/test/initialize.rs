...
fn initialize_pool() {
    ...
    msg!("pool_authority: {:#?}", pool_authority.pubkey());
    let pool_storage_account = Keypair::new();
    msg!("pool_storage_account: {:#?}", pool_storage_account.pubkey());
    ...
    rpc_client
        ...
        msg!("pool_storage {:#?}", pool_storage);
        
        assert_eq!(pool_storage.pool_authority, pool_authority.pubkey());
        assert_eq!(ool_storage.total_staked, 0);
